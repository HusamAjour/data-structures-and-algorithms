...
const fixTheTypo = () => {
  $('#fruits').children('li').eq(2).text('Pear');
};
...
const onlyOddChars = (str) => {
  ...
  let newStr = strArr.reduce((cumm,val,i)=>{
    if(i%2 !== 0){
      return cumm+val; 
    } else{
      return cumm;
...
const unenrollBrook = (arr) => {
  let newArr = arr.map(row=>{
    let rowEntries = row.filter(entriy=>{
      if(!entriy.includes('Brook')){
        return entriy;
      };
    });
    return rowEntries;
...
const sortByDay = (arr) => {
  ...
  daysOfWeek.forEach(day=>{
    let events = arr.reduce((cumm, val)=>{
      if(val.includes(day)){
        cumm.push(val);
        return cumm;
      }
