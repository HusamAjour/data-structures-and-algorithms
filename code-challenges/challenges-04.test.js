...
const generateSubmitButton = () => {
  let btn = $(`<button type='submit'>submit</submit>`);
  $('form').append(btn);
}
...
const citiesAtoJ = (arr) => {
  let regexTest = /^[A-J]/g;
  let cities = arr.filter(city => {
    if(regexTest.test(city)){
      console.log(regexTest.test(city));
      return city;
    }
  });
  return cities;
...
const noPunctuation = str => {
  ...
  let noPunc = strArr.filter(word=>{ 
    if(!regex.test(word)){
      return {word};
    };
  });
  let newNoPunc = noPunc.map(w =>{
    return w+' ';
  });
  return newNoPunc;
};
...
let hangman = (str) => {
  let newStr = str.replace(/[aieouAEIOU]/g, '_');
  return newStr;
};
/* ------------------------------------------------------------------------------------------------
...
const findShells = (str) => {
  ...
  let ellsArr = strArr.filter(word =>{
    if((/(ells)/g).test(word)){
      return word
    }
  });
