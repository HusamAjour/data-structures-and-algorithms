...
const templateWithJQuery = () => {
  starWarsPeople.forEach(character => {
    let section = $('#template').first().clone();
    section.find('h2').text(character.name);
    section.find('h3').text(character.height);
    section.find('p').text(character.eye_color);
    $('main').append(section);
  });
}
...
const listFoods = (recipe) => {
  ...
  ingr.forEach(ingredient => {
    result.push(ingredient.slice(ingredient.indexOf(' ', ingredient.indexOf(' ') + 1), ingredient.length).trim());
  });
  return result;
};
/* ------------------------------------------------------------------------------------------------
...
Write a function named stepAction that takes in the recipe and extracts the action verbs from the 
