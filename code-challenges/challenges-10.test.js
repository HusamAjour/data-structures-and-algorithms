...
const count = (target, input) => {
  /*let total = input.reduce((accumulator, arr) =>{
    let x = arr.filter((v) => {
      console.log(v);
  ...
  /*if(input.length !== 0){
  let total = input.reduce((cumm, arr, i)=>{
    if(arr.length ===0 || cumm.length===0){
      return cumm;
    } else if(Array.isArray(cumm) && cumm.length !==0){
       cumm = cumm.filter((v) => (v === target)).length;
       return cumm + arr.filter((v) => (v === target)).length;
     }else{
      return cumm + arr.filter((v) => (v === target)).length;
     }
  });
  ...
  if (input.length !== 0) {
    let total = input.reduce((cumm, arr) => {
      return cumm + arr.filter((v) => (v === target)).length;
    }, 0);
    return total;
  }
...
const totalSum = (input) => {
  let total = input.reduce((cumm, arr) => {
    return cumm + arr.reduce((v, arrVal) => (v + arrVal));
  }, 0);
  return total;
};
