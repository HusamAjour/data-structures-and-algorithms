...
const filterStringsWithVowels = (arr) => {
  ...
  let withVowels = arr.filter(word => {
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  console.log(arr);
  let baseStats = arr.filter(st => {
    if (st.baseStat > minBaseStat) {
      return st;
    }
  });
  return baseStats;
...
const getStatName = (arr, minBaseStat) => {
  let baseStat = []
  let newArr = arr.filter(st => {
    if (st.baseStat > minBaseStat) {
      baseStat.push(st.stat.name);
      return st.stat;
    }
  });
  return baseStat;
...
const getCharactersWithoutChildren = (arr) => {
  let noChildren = arr.filter(ch => {
    if (typeof ch.children === 'undefined') {
      return ch;
    }
  });
  return noChildren;
...
const evenOddNumericValues = (arr) => {
  let numerical = arr.filter(num => {
    if (typeof num === 'number') {
      return num;
    }
  });
  let parity = numerical.map(num => {
