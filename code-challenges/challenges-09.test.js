...
function getCurrentEvents(request, response) {
  let events = mapCurrentEvents();
  response.send(events);
}

const mapCurrentEvents = () => {
  let eventObjescts = currentEvents.news.map(e => {
    let eventObj = new Event(e);
    return eventObj;
  });
  return eventObjescts;
}
...
const countNumberOfElements = (arr) => {
  let arrayLen = arr.reduce((cumm, val, i) => {
    return i + 1;
  });
  return arrayLen;
};
...
const returnNames = (arr) => {
  let namesArray = arr.reduce((names, val, i) => {
    names.push(val.name);
    return names;
  }, []);
  return namesArray;
...
const reversedString = (str) => {
  ...
  let reversedStr = stringToArr.reduce((rev, val, i) => {
    rev.unshift(val);
    return rev;
  }, []).join('');
  return reversedStr;
...
const countNumberOfChildren = (arr) => {
  let countofChildren = arr.reduce((cumm, val) => {
    if (typeof val.children !== 'undefined') {
      return cumm + val.children.length;
    } else {
      return cumm;
    }
  }, 0);
...
const calculateAverage = (arr) => {
  let avg = arr.reduce((cumm, val, i) => {
    return ((cumm * i) + val) / (i + 1);
  }, 0);
  return avg;
};
/* ------------------------------------------------------------------------------------------------
...
const countPrimeNumbers = (arr) => {
  let primes = arr.reduce((cumm, val) => {
    if (isPrime(val)) { return cumm+1; }
    else { return cumm; }
  }, 0);
  return primes;
};
...
const extractStat = (statName, arr) => {
  let statObj = arr.reduce((cumm, val)=>{
...
const extractChildren = (arr) => {
  let nameHasA = arr.filter(ch => {
    if(ch.name.includes('a')){return ch;}
  });

  let names = nameHasA.reduce((cumm, val)=>{
